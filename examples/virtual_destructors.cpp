/**
 * Пример использование constructors, освещающий некоторые аспекты работы
 * виртуальных функций, а также методы корректного удаления объектов
 * полиморфных классов
 */
...
MAIN
    ...
    delete dA;  // Это некорректное удаление объектов, потому нужно привести
                // счетчики числа объектов классов в порядок перед дальнейшим
                // использованием примера:
                       B::resetCount();
                       C::resetCount();
                       D::resetCount();
